...
//	@FXML private ToggleSwitch switchCompact;
	@FXML private ToggleButton tglCompact;
	...
	public void initialize(Stage currentStage)
	{
		...
		tglCompact.setSelected(false);
		pswKey.setFont(new Font("Noto Sans", 12));
		pswPassword.setFont(new Font("Noto Sans", 12));
	}
	...
	protected void btnEncrypt_Clicked(ActionEvent event) throws IOException
	{
		try {
			...
			CryogenDialogs error = new CryogenDialogs(Alert.AlertType.ERROR, "Error", null, ex.getMessage());
			error.showAndWait();
		}
	}
	...
	*/
	public static char[] encrypt(char[] userPassword, char[] key, int limit)
	{
		try {
			...
			int ext = -1;
			if (i > 32) {
				CryogenDialogs confirm = new CryogenDialogs(Alert.AlertType.CONFIRMATION, "Password is greater than 32 characters", "Would you like to shorten the password to the 32 limit?", null);
			...
			CryogenDialogs error = new CryogenDialogs(Alert.AlertType.ERROR, "Error", "Failed to Encrypt Password", ex.getMessage());
			error.showAndWait();
